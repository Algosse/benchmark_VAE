...
""" Helpers functions from https://github.com/openai/vdvae/blob/main/vae_helpers.py to use discretized_mix_logistic_loss """
...
class DmolNet(nn.Module):
    """ Output net for HVAE model that samples from decoder's output distribution """
    ...
    def nll(self, px_z, x):
        return discretized_mix_logistic_loss(x=x.permute(0, 2, 3, 1), l=self.forward(px_z), low_bit=False)
    ...
    def sample(self, px_z):
        ...
        return xhat.permute(0, 3, 1, 2).int()